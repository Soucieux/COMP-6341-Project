...
# 2. Initialize pygame board to draw lines
...
def start():
    ...
    for x in range(gray_image.width - 1):
        for y in range(gray_image.height - 1):
            brightSum += gray_image.getpixel((x, y))
    ...
    # Draw all lines
    draw(gray_image, brightSum, image_size, screen)
...
# 2. Loop through each pixel to find the lines
def draw(gray_image, brightness_sum, image_size, screen):
    ...
    # Apply the result of canny edge onto the pygame screen
    surface = pygame.surfarray.make_surface(canny_edge)
    ...
    for y in range(gray_image.height - 1):
        ...
        for x in range(gray_image.width - 1):
            ...
            average_brightness = brightness_sum / image_size
